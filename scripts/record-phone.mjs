...
try {
  ...
  // ---- per-stop swap, in the real UI --------------------------------------
  await step("per-stop swap", async () => {
    const btn = page.locator(".day-stops .swap-stop-btn").nth(1);
    await btn.scrollIntoViewIfNeeded();
    await beat(900);
    await btn.click();
    await page.waitForSelector(".alt-list .alt-row", { timeout: 10000 });
    await beat(2800); // let the ranked alternatives sit on screen
    await page.locator(".alt-list .alt-row").first().click();
    await page.waitForSelector(".stop-swapped", { timeout: 10000 });
    await beat(3600); // swapped badge + the cost / travel delta chips
  });

  // ---- S2: the destination taste deck --------------------------------------
  await step("destination taste deck", async () => {
    await page.locator(".dest-deck-btn").click();
    await page.waitForSelector(".dest-sheet .swipe-card.live", { timeout: 15000 });
    await beat(2400);
    for (const act of [".act-like", ".act-must", ".act-like"]) {
      if (!(await page.locator(".dest-sheet .deck-actions").count())) break;
      await page.locator(`.dest-sheet ${act}`).click();
      await beat(760);
    }
    await beat(1600);
    await page.locator(".dest-sheet .back").first().click();
    await beat(1400);
  });
