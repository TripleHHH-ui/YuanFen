...
> plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. This plan is also the
...
- [ ] Root scripts: `dev` (concurrently api+web), `test` (vitest run in shared+api), `fareboard`.
- [ ] Verify: `npm install` clean; `npm test` runs (0 tests).
...
- [ ] Verify: JSON parses; every place has coords within city bbox + 7-day openHours; matrix is
...
- [ ] Tests first: vibe-pick seeding (5 tags → positive weights); `applySwipe` like/+, pass/−,
...
- [ ] Implement minimal `taste.ts`; all green.

### Task 4: Shared engine — long weekends + fare ranking (TDD)
**Files:** `packages/shared/src/{calendar.ts,fareboard.ts}`, tests.
- [ ] Tests: `longWeekends(holidays, from)` finds Deepavali Sat07–Mon09 Nov 2026 from 2026-08-24;
...
- [ ] Tests: `buildDays` — 09:30 arrival → full day-1 (≥3 stops); 23:40 arrival → night-food stop
      ...
      money change.
- [ ] Implement greedy slot-filler over taste-ranked open places + template narrator; green.
...
`test/atlas.test.ts`.
- [ ] Envelope types mirror cli-contract.md; `FixtureAtlasClient` serves fixtures, generates
...
- [ ] Tests: fixture search returns offers; evidence grows; passenger details never in log output.
...
- [ ] `intent.ts`: deterministic parser — city/area match, "must (eat|see|visit) X" clauses, mood
...
- [ ] Tests: S1 phrase → CBD day plan JSON with chicken-rice stop + quiet closer; alert endpoint
...
- [ ] 12 tags, Continue gated at 5 (FR-002); 15-card deck with drag/buttons, like/pass/must-go,
...
- [ ] Carto Positron basemap desaturated; numbered stops + leg times; chat bar drives /api/plan/chat;
...
- [ ] Unprompted alert banner → hand of 3+sealed wildcard, FIXTURE badge, total-with-bag headline,
...
- [ ] Shared state machine: verify → price-change reconfirm → masked summary → "I approve this
...
- [ ] `npm test` all green; `npm run dev`; walk the golden path in browser (Playwright screenshots):
...
- [ ] `npm run fareboard` writes dated snapshot; badge logic reads snapshot history count, fixture
